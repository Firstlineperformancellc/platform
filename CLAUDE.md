...
Supabase: FLP org "First Line Performance LLC" (pyrnphumzvmzfaruamno). Dev project `flp-platform-dev` (ref uspbmbvoxotbelribjgo, us-east-1, free), linked from this repo; apply migrations with `npx supabase db query --linked -f supabase/migrations/<file>` and record the version in `supabase_migrations.schema_migrations` (no DB password needed). The deck uses the org's other project (nkctthacwjvxytyvndxx). Production project comes at promotion (Pro). GitHub: github.com/Firstlineperformancellc with repos `platform` (this repo, `main` + `dev`) and `deck` (`dev`). Push through the SSH host alias `github.com-flp`, key `~/.ssh/flp_github`, which is registered on the FLP GitHub account; Scott's personal GitHub login is never used for FLP. DigitalOcean account admin@firstlineperform.com, doctl context `flp` (default). Droplet `flp-web-01` (Ubuntu 26.04, NYC3, 157.245.213.222), `ssh flp-web` as user `flp`, SSH key `~/.ssh/flp_do`. Dev droplet `flp-dev-01` (Ubuntu 26.04, NYC3, 159.65.46.178, project "First Line Performance Development"), `ssh flp-dev` as user `flp`; serves `dev.firstlineperform.com` (static web export at /var/www/dev.firstlineperform.com/html, deploy with `apps/product/deploy/deploy-dev.sh`) and `api-dev.firstlineperform.com` (nginx -> 127.0.0.1:3200, systemd `flp-api`, deploy with `services/api/deploy/deploy-dev.sh`, env at /var/www/flp-api/.env copied from the gitignored `services/api/.env.dev`). Node 24 at /usr/local/bin on both boxes. Cloud firewall `flp-web` by tag `flp` covers both droplets. Let's Encrypt with auto-renew. DNS at GoDaddy. Google Workspace with admin@ and aliases.
