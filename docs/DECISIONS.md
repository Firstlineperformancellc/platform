...
- **Mail stays on Google Workspace.** Inbound is a Google Apps Script in the support@ mailbox (`deploy/support/inbound-mail.gs`) that posts each new message to `POST /support/inbound` with a shared secret every two minutes; replies from customers thread onto their ticket by the `[FLP-n]` subject tag, then by In-Reply-To/References, then by an open ticket from the same sender. Duplicates are dropped by Message-ID; automated senders are ignored. Attachments are recorded by name and stay in the mailbox.
